...
The current upstream app exposes a machine-readable release feed at `https://persistent.oaistatic.com/codex-app-prod/appcast.xml`. That feed points to versioned macOS ZIP artifacts such as `Codex-darwin-arm64-26.422.71525.zip`, which are simpler to automate against than the generic `Codex.dmg` URL.
...
```bash
npm run extract-automation-examples -- --version 26.422.71525
```

Install the current build as a desktop app for the current user:

```bash
npm run install-desktop -- --version 26.422.71525
```

Build a specific upstream release:

```bash
npm run repackage -- --version 26.422.71525
```

Build from a local upstream ZIP:

```bash
npm run repackage -- --zip /path/to/Codex-darwin-arm64-26.422.71525.zip
```
...
```bash
npm run smoke-test -- --version 26.422.71525
```
